...
export const UserList: FC = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      setList(users);
      const initialCheckedState = Object.fromEntries(users.map((user: any) => [user._id, false]));
  ...
  const handleSelectAllChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    const updatedState = Object.fromEntries(list.map((user) => [user._id, isChecked]));
            ...
            <tr key={user._id} className="align-middle text-center">
              ...
              <td className="d-flex gap-1 justify-content-center align-items-center">
                <Button
                  variant="info"
                  className="me-2"
                  onClick={() => navigate(`/dashboard/manage-user/detail-user/${user._id}`)}
                >
                  Detail
                </Button>
                <ButtonPage
